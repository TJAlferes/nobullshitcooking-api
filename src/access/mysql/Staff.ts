...
export class Staff implements IStaff {
  ...
  async getByEmail(email: string) {
    const sql = `SELECT id, email, pass, staffname, confirmation_code FROM staff WHERE email = ?`;
    const [ [ row ] ] = await this.pool.execute<RowDataPacket[]>(sql, [email]);
    return row;
  }

  async getByName(staffname: string) {
    const sql = `SELECT id, email, pass, staffname, confirmation_code FROM staff WHERE staffname = ?`;
  ...
  async create({ email, pass, staffname, confirmationCode }: ICreatingStaff) {
    const sql = `INSERT INTO staff (email, pass, staffname, confirmation_code) VALUES (?, ?, ?, ?)`;
    const [ [ row ] ] = await this.pool.execute<RowDataPacket[]>(sql, [email, pass, staffname, confirmationCode]);
    return row;
  }

  async verify(email: string) {
    const sql = `UPDATE staff SET confirmation_code = NULL WHERE email = ? LIMIT 1`;
    const [ [ row ] ] = await this.pool.execute<RowDataPacket[]>(sql, [email]);
    return row;
  }

  async update({ id, email, pass, staffname }: IUpdatingStaff) {
    const sql = `UPDATE staff SET email = ?, pass = ?, staffname = ? WHERE id = ? LIMIT 1`;
    const [ [ row ] ] = await this.pool.execute<RowDataPacket[]>(sql, [email, pass, staffname, id]);
    return row;
  }
