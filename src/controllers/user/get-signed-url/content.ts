...
const AWS_NOBSC_USER_CONTENT_S3_BUCKET: string =
process.env.AWS_NOBSC_USER_CONTENT_S3_BUCKET!;

export async function getSignedUrlContent(req: Request, res: Response) {
  const fileNameFullSize = `${req.session!.userInfo.username}-${uuidv4()}`;
  const fileNameThumbSize = `${fileNameFullSize}-thumb`;
  const fileType = req.body.fileType;

  const s3 = new S3({
    accessKeyId: process.env.AWS_NOBSC_USER_CONTENT_ACCESS_KEY_ID,
    secretAccessKey: process.env.AWS_NOBSC_USER_CONTENT_SECRET_ACCESS_KEY
  });

  const signatureFullSize = await getSignedUrlPromise(s3, 'putObject', {
    Bucket: AWS_NOBSC_USER_CONTENT_S3_BUCKET,
    Key: fileNameFullSize,
    ContentType: fileType,
    Expires: 50
  });

  const signatureThumbSize = await getSignedUrlPromise(s3, 'putObject', {
    Bucket: AWS_NOBSC_USER_CONTENT_S3_BUCKET,
