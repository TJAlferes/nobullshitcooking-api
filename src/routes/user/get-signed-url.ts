...
export function userGetSignedUrlRouter() {
  ...
  router.post('/recipe-cooking',     userIsAuth, [body('fileType').not().isEmpty().trim().escape()], catchExceptions(getSignedUrl.recipeCooking));
  router.post('/recipe-equipment',   userIsAuth, [body('fileType').not().isEmpty().trim().escape()], catchExceptions(getSignedUrl.recipeEquipment));
  router.post('/recipe-ingredients', userIsAuth, [body('fileType').not().isEmpty().trim().escape()], catchExceptions(getSignedUrl.recipeIngredients));
