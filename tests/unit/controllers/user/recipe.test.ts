...
const controller = new UserRecipeController({} as Pool);
...
jest.mock('../../../../src/access/mysql', () => ({
  ...
  RecipeEquipment: jest.fn().mockImplementation(() => ({
    create:           RECreate,
    update:           REUpdate,
    deleteByRecipeId: REDeleteByRecipeId
  })),
  RecipeIngredient: jest.fn().mockImplementation(() => ({
    create:           RICreate,
    update:           RIUpdate,
    deleteByRecipeId: RIDeleteByRecipeId
  })),
  RecipeMethod: jest.fn().mockImplementation(() => ({
    create:           RMCreate,
    update:           RMUpdate,
    deleteByRecipeId: RMDeleteByRecipeId
  })),
...
describe('user recipe controller', () => {
  ...
  describe ('update method', () => {
    ...
    const req: Partial<Request> =  {session, body: {recipeInfo: {id: "1", ownership: "private", ...allRecipeInfo}}};
    const res: Partial<Response> = {send: jest.fn().mockResolvedValue({message})};
    ...
    it('uses assert on recipe equipment', async () => {
      await controller.update(<Request>req, <Response>res);
      expect(assert).toHaveBeenCalledWith(assertEquipment[0], validRecipeEquipment);
      expect(assert).toHaveBeenCalledWith(assertEquipment[1], validRecipeEquipment);
    });

    it('uses RecipeEquipment.create', async () => {
      await controller.update(<Request>req, <Response>res);
      expect(RECreate).toHaveBeenCalledWith('(?, ?, ?),(?, ?, ?)', [1, 1, 1, 1, 2, 2]);
    });

    it('uses assert on recipe ingredients', async () => {
      await controller.update(<Request>req, <Response>res);
      expect(assert).toHaveBeenCalledWith(assertIngredients[0], validRecipeIngredient);
      expect(assert).toHaveBeenCalledWith(assertIngredients[1], validRecipeIngredient);
    });

    it('uses RecipeIngredients.create', async () => {
      await controller.update(<Request>req, <Response>res);
      expect(RICreate).toHaveBeenCalledWith('(?, ?, ?, ?),(?, ?, ?, ?)', [1, 1, 1, 1, 1, 2, 2, 2]);
    });

    it('uses assert on recipe subrecipes', async () => {
      await controller.update(<Request>req, <Response>res);
      expect(assert).toHaveBeenCalledWith(assertSubrecipes[0], validRecipeSubrecipe);
      expect(assert).toHaveBeenCalledWith(assertSubrecipes[1], validRecipeSubrecipe);
    });
