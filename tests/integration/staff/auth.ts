...
export function staffAuthTests() {
  describe('POST /staff/auth/register', () => {
    const args =
      {email: "newstaff@site.com", pass: "secret", staffname: "newuser"};
    
    it('registers new staff', async () => {
      const { body } =
        await request(server).post('/staff/auth/register').send(args);
  ...
  describe('POST /staff/auth/resend-confirmation-code', () => {
    ...
    it('resends confirmation code', async () => {
      const { body } = await request(server)
        .post('/staff/auth/resend-confirmation-code')
        .send(args);
  ...
  describe('POST /staff/auth/verify', () => {
    ...
    it('verifies new staff', async () => {
      const { body } =
        await request(server).post('/staff/auth/verify').send(args);
